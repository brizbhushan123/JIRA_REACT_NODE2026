...
const CustomFieldValue = {
    async create(data) {
        const result = await db_1.pool.query(`INSERT INTO customfieldvalue (issue_id, customfield_id, stringvalue, numbervalue, textvalue, datevalue)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING *`, [
            data.issueId,
    ...
    async findOne(filter = {}) {
        ...
        let query = 'SELECT * FROM customfieldvalue';
        const values = [];
        if (keys.length > 0) {
    ...
    async findByIssue(issueId) {
        const result = await db_1.pool.query('SELECT * FROM customfieldvalue WHERE issue_id = $1 ORDER BY id ASC', [issueId]);
        return result.rows.map(r => (0, dbUtils_1.toCamelCase)(r));
    },
    async delete(id) {
        const result = await db_1.pool.query('DELETE FROM customfieldvalue WHERE id = $1', [id]);
