...
const Label = {
    async create(data) {
        const result = await db_1.pool.query(`INSERT INTO label (issue_id, label) VALUES ($1, $2) RETURNING *`, [data.issueId, data.label ?? null]);
    ...
    async findByIssue(issueId) {
        const result = await db_1.pool.query('SELECT * FROM label WHERE issue_id = $1 ORDER BY id ASC', [issueId]);
        return result.rows.map(r => (0, dbUtils_1.toCamelCase)(r));
    },
    async delete(id) {
        const result = await db_1.pool.query('DELETE FROM label WHERE id = $1', [id]);
