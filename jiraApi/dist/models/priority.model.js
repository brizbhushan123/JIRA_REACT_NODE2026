...
const Priority = {
    async create(data) {
        const result = await db_1.pool.query(`INSERT INTO priority (pname, description) VALUES ($1, $2) RETURNING *`, [data.pname, data.description ?? null]);
    ...
    async findById(id) {
        const result = await db_1.pool.query('SELECT * FROM priority WHERE id = $1', [id]);
        return result.rows[0] ? (0, dbUtils_1.toCamelCase)(result.rows[0]) : null;
    },
    async findAll() {
        const result = await db_1.pool.query('SELECT * FROM priority ORDER BY id ASC');
