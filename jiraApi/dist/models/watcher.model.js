...
const Watcher = {
    async create(data) {
        const result = await db_1.pool.query(`INSERT INTO watcher (issue_id, user_id) VALUES ($1, $2) RETURNING *`, [data.issueId, data.userId ?? null]);
    ...
    async findByIssue(issueId) {
        const result = await db_1.pool.query('SELECT * FROM watcher WHERE issue_id = $1 ORDER BY id ASC', [issueId]);
        return result.rows.map(r => (0, dbUtils_1.toCamelCase)(r));
    },
    async delete(id) {
        const result = await db_1.pool.query('DELETE FROM watcher WHERE id = $1', [id]);
